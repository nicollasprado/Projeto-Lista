...
bool test_capacity(int* error_buffer){
    ArrayList al;
    ...
    if(al.capacity() == 128){
        ...
        error_buffer[0] = 128;
        error_buffer[1] = al.capacity();
    }

    return passed;
}

bool test_percent_occupied(int* error_buffer){
    ArrayList al;
    ...
    bool passed = false;
    if(al.percent_occupied() == 0.78125){
        passed = true;
    }else{
        error_buffer[0] = 78125;
        error_buffer[1] = al.percent_occupied();
    }
...
bool test_find_all(int* error_buffer){
    ...
    ArrayList found = al.find_all(49);
...
bool test_remove_quantity(int* error_buffer){
    ArrayList al;
    ...
    al.remove_quantity(25, 2);

    bool passed = false;
    if(al[25] != 25 && al[26] != 25){
        passed = true;
    }else{
        error_buffer[0] = 26;
        error_buffer[1] = al[25];
        error_buffer[2] = 26;
        error_buffer[3] = al[25];
    }
...
int choose_option(int* error_buffer, int chosen_test){
    ...
    switch(chosen){
        ...
        case 4:
            ...
            break;
        case 5:
            if(test_percent_occupied(error_buffer)){
                std::cout << "\npercent_occupied test OK! \n" << std::endl;
            }else{
                std::string error_msg = error_process(error_buffer);
                std::cout << "\npercent_occupied test failed...\n" << error_msg << "\n" << std::endl;
            }
            break;
        case 6:
        ...
        case 15:
            ...
            break;
        case 16:
            if(test_remove_quantity(error_buffer)){
                std::cout << "\nremove_quantity test OK! \n" << std::endl;
            }else{
                std::string error_msg = error_process(error_buffer);
                std::cout << "\nremove_quantity test failed...\n" << error_msg << "\n" << std::endl;
            }
            break;
        case 17:
